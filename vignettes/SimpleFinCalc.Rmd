...
Initially, the SimpleFinCalc package uses S3 class objects, such as the data.frame returned by amort_schedule(), for simplicity and ease of implementation. Looking forward, however, I could transition the output of functions like amort_schedule() to declare and create S4 class objects. This change would leverage S4's more formal and rigorous framework with defined slots and data types, allowing for fine-tuning data attributes, enforcing constraints on input values like interest rates and loan terms, and creating more robust, maintainable, and self-documenting code in future releases.
...
The fv_simple() function performs a 'Simple Future Value' calculation with robust input 
...
The fv_compound() function performs a 'Compound Future Value' calculation with robust input 
...
The pv() function performs a 'Present Value' calculation with robust input validation.
...
The annuity_payment() function performs an 'annuity_payment' calculation with robust input validation.
...
The loan_payment() function performs a 'loan_payment' calculation with robust input validation.
It takes three variables; Principal, Rate, and Years; 
...
The 'amort_schedule()' function performs 'amort_schedule' calculations with robust input validation.
It takes three variables; Principal, Rate, and Years; 
...
The cagr() function performs a 'cagr' calculation with robust input validation.
...
The simple_return() function performs a 'simple_return' calculation with robust input validation.
